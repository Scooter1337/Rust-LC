...
- `make run -m`, which will engage Manual Mode, where you can type in expressions and press enter to parse them. It will exit when you type `exit` or `quit`.
- `make run -b "{expr}" {times}`, which will run the benchmark. Example usage: `make run -b "a b c" 1000`
